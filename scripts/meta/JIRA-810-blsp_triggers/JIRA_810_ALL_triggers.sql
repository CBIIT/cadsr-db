...
GRANT INSERT, SELECT, UPDATE ON SBR.TRRG_TRM_SPC_ERROR_LOG TO SBREXT;
/
...
EXCEPTION
     WHEN OTHERS THEN
      errmsg := 'DETL_NAME:'||:new.DETL_NAME||';'||substr(SQLERRM,1,100);     
       insert into SBR.TRRG_TRM_SPC_ERROR_LOG VALUES('BR.DTL_TL_BIU_ROW_TRM_SPC','SBR.DESIG_TL_BIU_ROW_TRM_SPC', sysdate ,errmsg);
      RAISE_APPLICATION_ERROR(-20001,errmsg );
...
EXCEPTION
     WHEN OTHERS THEN
      errmsg := 'DTL_NAME:'||:new.DTL_NAME||';'||substr(SQLERRM,1,100);     
       insert into SBR.TRRG_TRM_SPC_ERROR_LOG VALUES('BR.DTL_TL_BIU_ROW_TRM_SPC','SBR.DATATYPES_LOV', sysdate ,errmsg);
      RAISE_APPLICATION_ERROR(-20001,errmsg );

END;
CREATE OR REPLACE TRIGGER SBR.FRL_BIU_ROW_TRM_SPC
BEFORE INSERT OR UPDATE ON SBR.FORMATS_LOV
FOR EACH ROW
DECLARE
...
BEGIN 
                  :new.DESCRIPTION:= regexp_replace(:new.DESCRIPTION, '(^([[:space:]]|[[:cntrl:]])+)|(([[:space:]]|[[:cntrl:]])+$)',null);
                  :new.FORML_NAME:= regexp_replace(:new.FORML_NAME, '(^([[:space:]]|[[:cntrl:]])+)|(([[:space:]]|[[:cntrl:]])+$)',null);

EXCEPTION
     WHEN OTHERS THEN
      errmsg := 'FORML_NAME:'||:new.FORML_NAME||';'||substr(SQLERRM,1,100);     
       insert into SBR.TRRG_TRM_SPC_ERROR_LOG VALUES('SBR.FRL_BIU_ROW_TRM_SPC','SBR.FORMATS_LOV', sysdate ,errmsg);
      RAISE_APPLICATION_ERROR(-20001,errmsg );
...
END;
CREATE OR REPLACE TRIGGER SBREXT.QAT_BIU_ROW_TRM_SPC
BEFORE INSERT OR UPDATE ON SBREXT.QUEST_ATTRIBUTES_EXT
FOR EACH ROW
DECLARE

errmsg VARCHAR2(200);
BEGIN 
         :new.DEFAULT_VALUE:= regexp_replace(:new.DEFAULT_VALUE, '(^([[:space:]]|[[:cntrl:]])+)|(([[:space:]]|[[:cntrl:]])+$)',null);
        
EXCEPTION
     WHEN OTHERS THEN
      errmsg := 'QUEST_IDSEQ:'||:new.QUEST_IDSEQ||';'||substr(SQLERRM,1,100);     
       insert into SBR.TRRG_TRM_SPC_ERROR_LOG VALUES('SBREXT.QAT_BIU_ROW_TRM_SPC','SBREXT.QUEST_ATTRIBUTES_EXT', sysdate ,errmsg);
      RAISE_APPLICATION_ERROR(-20001,errmsg );
...
END;
CREATE OR REPLACE TRIGGER SBREXT.QUEST_BIU_ROW_TRM_SPC
BEFORE INSERT OR UPDATE ON SBREXT.QUEST_VV_EXT
FOR EACH ROW
DECLARE
...
EXCEPTION
     WHEN OTHERS THEN
      errmsg := 'QV_IDSEQ:'||:new.QV_IDSEQ||';'||substr(SQLERRM,1,100);     
       insert into SBR.TRRG_TRM_SPC_ERROR_LOG VALUES('SBREXT.QUEST_VV_EXT','SBREXT.QUEST_VV_EXT', sysdate ,errmsg);
      RAISE_APPLICATION_ERROR(-20001,errmsg );
