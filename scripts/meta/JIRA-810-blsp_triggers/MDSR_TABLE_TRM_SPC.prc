CREATE OR REPLACE PROCEDURE SBR.MDSR_TABLE_TRM_SPC as  
...
FOR rec IN c_table LOOP  
    ...
    /*DBMS SQL*/
    EXECUTE IMMEDIATE 'SELECT length('||rec.column_name||') ,'||rec.pk_column||','||rec.column_name  ||'  FROM ' || rec.table_name|| ' WHERE  date_created >sysdate-240 and REGEXP_like('||rec.column_name|| ','||'''(^([[:space:]]|[[:cntrl:]])+)|(([[:space:]]|[[:cntrl:]])+$)'''||')'
    BULK COLLECT INTO t;    
    ...
    FOR i IN 1 .. t.COUNT LOOP
        ...
        insert into SBR.TRRG_TRM_SPC_ERROR_LOG VALUES('SBR.MDSR_TABLE_TRM_SPC',rec.table_name||'.'||rec.column_name, sysdate ,errmsg);
        commit;
