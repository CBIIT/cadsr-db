drop TABLE SBREXT.META_CONCEPTS_EXT_TEMP
/
drop TABLE SBREXT.META_CONCEPTS_EXT_ERROR_LOG
/
CREATE TABLE SBREXT.META_CONCEPTS_EXT_TEMP
(    PREFERRED_NAME        VARCHAR2(300 BYTE)      ,
  LONG_NAME             VARCHAR2(500 BYTE),
  PREFERRED_DEFINITION  VARCHAR2(3000 BYTE)    
 )
/
CREATE TABLE SBREXT.META_CONCEPTS_EXT_ERROR_LOG
...
cursor c_load is 
 ...
 (select count(*) cnt,trim(PREFERRED_NAME) PREFERRED_NAME,  trim(LONG_NAME) LONG_NAME,
  trim(PREFERRED_DEFINITION)PREFERRED_DEFINITION from SBREXT."META_CONCEPTS_EXT_TEMP"
