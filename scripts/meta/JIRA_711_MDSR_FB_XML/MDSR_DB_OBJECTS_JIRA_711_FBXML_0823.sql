...
CREATE OR REPLACE TYPE   MDSR_INSTRUCTIONS_T as object("text" VARCHAR2(2000))
/
...
,"instruction"    MDSR_INSTRUCTIONS_T
...
,"type"              VARCHAR2(5) 
,"headerInstruction"     MDSR_INSTRUCTIONS_T                 
,"footerInstruction"     MDSR_INSTRUCTIONS_T  
,"module_xx" MDSR_FB_MODULE_XML_LIST_T1
...
AS
   ...
   ORDER BY cs.CS_ID;
 /
CREATE MATERIALIZED VIEW MDSR_FB_QUEST_MODULE_MVW
...
/
CREATE INDEX MDSR_FB_MODF_ID_INX ON SBREXT.MDSR_FB_QUEST_MODULE_MVW
(CRF_IDSEQ)
/
CREATE INDEX MDSR_FB_MOD_ID_INX ON SBREXT.MDSR_FB_QUEST_MODULE_MVW
(MOD_IDSEQ)
/
CREATE MATERIALIZED VIEW MDSR_FB_QUESTION_MVW 
ENABLE QUERY REWRITE 
AS 
...
/
CREATE INDEX MDSR_FB_QDE_ID_INX ON MDSR_FB_QUESTION_MVW
(DE_IDSEQ)
/
CREATE INDEX MDSR_FB_QF_ID_INX ON MDSR_FB_QUESTION_MVW
(FORM_IDSEQ)
/
CREATE INDEX MDSR_FB_QM_ID_INX ON MDSR_FB_QUESTION_MVW
(MOD_IDSEQ)
/
CREATE INDEX MDSR_FB_QVD_ID_INX ON MDSR_FB_QUESTION_MVW
(VD_IDSEQ)
/
CREATE INDEX MDSR_FB_Q_ID_INX ON MDSR_FB_QUESTION_MVW
(QUES_IDSEQ)   
/   
CREATE MATERIALIZED VIEW MDSR_FB_VALID_VALUE_MVW 
ENABLE QUERY REWRITE 
AS 
SELECT QF.qc_id FORM_ID,
  ...
  FROM QUEST_CONTENTS_EXT QV,
       ...
       (SELECT VP_IDSEQ, vm.vm_id vm_id, vm.version vm_version
          FROM SBR.VALUE_MEANINGS vm, SBR.PERMISSIBLE_VALUES pv, SBR.VD_PVS vp
...
AS
   SELECT cf.name "context",
          ...
          FR.QTL_NAME "type",
          MDSR_INSTRUCTIONS_T (FI.preferred_definition),
          MDSR_INSTRUCTIONS_T (FF.preferred_definition),
          CAST (
             MULTISET (
                  SELECT MM.DISPLAY_ORDER,
                         NVL (mm.REPEAT_NO, 0),
                         MM.long_name,
                         MDSR_INSTRUCTIONS_T (Mod_instruction),
                         ...
                         CAST (
                                 ...
                                 SELECT                      --0level QUESTION
                                       QQ.QUES_id,
                                        ...
                                        DECODE (
                                           ...
                                           "dateModified",
                                        qq.Q_LONG_NAME,
                                        MDSR_INSTRUCTIONS_T (Q_instruction),
                                              ...
                                              CAST (
                                                 MULTISET (
                                                      SELECT c.long_name,
                                                             ...
                                                             || c.preferred_name
                                                                AS url
                                                        FROM SBREXT.COMPONENT_CONCEPTS_EXT cx,
                                                                 ...
                                                                 MULTISET (
                                                                    SELECT d.created_by,
                                                                     ...
                                                                     WHERE     c.CONTE_IDSEQ =
                                                                                  d.CONTE_IDSEQ
                                                                           AND d.ac_idseq =
                                                                                  vm2.vm_idseq) AS MDSR_FB_DESIGN_XML_LIST_T1),
                                                              CAST (
                                                                 MULTISET (
                                                                    SELECT df.created_by,
                                                                     ...
                                                                     WHERE     c.CONTE_IDSEQ(+) =
                                                                                  df.CONTE_IDSEQ
                                                                           AND df.ac_idseq =
                                                                                  vm2.vm_idseq) AS MDSR_FB_DEFIN_XML_LIST_T1),
                            ...
                            AS Question_LIST
                    FROM MDSR_FB_QUEST_MODULE_MVW mm
                   WHERE FR.qc_IDSEQ = mm.CRF_IDSEQ
          ...
          CAST (
             MULTISET (
                SELECT pe.lead_org,
                  ...
                  FROM SBREXT.PROTOCOLS_EXT pe, SBREXT.protocol_qc_ext pq, SBR.contexts c2
     ...
     FROM SBREXT.QUEST_CONTENTS_EXT FR,
          SBR.contexts cf,
           (select a.DN_CRF_IDSEQ,a.preferred_definition from
          (SELECT min(qc_id) OVER(PARTITION BY DN_CRF_IDSEQ ) as minqc_id,qc_id,qc_idseq,DN_CRF_IDSEQ,preferred_definition
           FROM QUEST_CONTENTS_EXT 
            WHERE QTL_NAME = 'FORM_INSTR'  )a
            where a.minqc_id=a.qc_id         
            ) fi, 
          (SELECT DN_CRF_IDSEQ, preferred_definition
             FROM QUEST_CONTENTS_EXT
