...
AS 
/* Formatted on 4/5/2018 11:45:30 AM (QP5 v5.269.14213.34769) */
...
/
CREATE INDEX MDSR_FB_MODF_ID_INX ON MDSR_FB_QUEST_MODULE_MVW
(CRF_IDSEQ);
/
CREATE INDEX MDSR_FB_MOD_ID_INX ON MDSR_FB_QUEST_MODULE_MVW
(MOD_IDSEQ);
/
CREATE MATERIALIZED VIEW MDSR_FB_QUESTION_MVW 
AS 
/* Formatted on 4/5/2018 11:50:04 AM (QP5 v5.269.14213.34769) */
...
AS 
/* Formatted on 4/5/2018 11:53:46 AM (QP5 v5.269.14213.34769) */
...
AS
   SELECT cf.name "context",
                                              ...
                                              CAST (
                                                 MULTISET (      --2d data set
                                                    SELECT pv.VALUE,
                                                           ...
                                                           --,DECODE ( pv.begin_date, NULL, NULL,TO_CHAR ( pv.begin_date,'YYYY-MM-DD')||'T'||TO_CHAR( pv.begin_date, 'HH24:MI:SS')||'.0') "begindate",
                                                           NULL
    ...
    WHERE     FR.ASL_NAME = 'RELEASED'
          ...
          AND (FR.QTL_NAME = 'CRF' OR FR.QTL_NAME = 'TEMPLATE')
          AND FR.qc_id IN (3065877,  5590324,  2263415, 2262683,
                           3443682, 3691952, 5791100,  4964471,
                           2019334, 2019339,2019340, 2019343,
                           2019346, 4118188, 4861838,  5317151,
                           3476190, 4750588,5314951,3476190,
                           5881710,3915131,4903131,5108287);
CREATE TABLE MDSR_FB_XML_TEMP
(
...
/
CREATE TABLE MDSR_FB_XML_REPORT_ERR
(
...
BEGIN 
        ...
        SELECT dbms_xmlgen.getxml( 'select*from MDSR_FB_XML_VW where "publicid" like'||''''||'%'||l_pid||'%'||'''')--='||l_pid||'''')--||'and version='||rec.VERSION  )--   where "collectionName" like'||''''||'%'||v_protocol||'%'||'''')
