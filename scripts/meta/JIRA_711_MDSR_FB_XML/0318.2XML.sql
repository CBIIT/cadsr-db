...
qq.D_PREFERRED_DEFINITION, 
   ...
   and vd.VD_IDSEQ=vp.VD_IDSEQ)as MDSR_FB_PV_XML_LIST_T5)  
   from value_domains vd 
    where  vd.vd_idseq=qq.vd_idseq) as MDSR_FB_VD_XML_LIST_T5),
    cast(multiset(  
  ...
  ,
      CAST (
                                                       ...
                                                       ) as  MDSR_FB_QUESTION_XML_LIST_T5)
                                                       
                                                       
                                                        from 
                            SBREXT.MDSR_FB_QUEST_MODULE_MVW mm
                            where mf.qc_IDSEQ=mm.CRF_IDSEQ                                                     
                            order by MM.DISPLAY_ORDER)as MDSR_FB_MODULE_XML_LIST_T5 ),
                      cast(multiset(
                                        SELECT  pe.lead_org,
                                        pe.phase,
                                        pe.type,
