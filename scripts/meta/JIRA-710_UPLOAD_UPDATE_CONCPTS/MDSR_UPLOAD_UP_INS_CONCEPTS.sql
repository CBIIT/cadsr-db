CREATE OR REPLACE PROCEDURE SBREXT.MDSR_UPLOAD_UP_INS_CONCEPTS AS

cursor c_load is
  select PREFERRED_NAME ,  LONG_NAME ,  PREFERRED_DEFINITION,EVS_SOURCE, ORIGIN, SOURCE_DEFINITION
    from
    (select distinct trim(T.PREFERRED_NAME) PREFERRED_NAME,  trim(LONG_NAME) LONG_NAME,
    trim(PREFERRED_DEFINITION)PREFERRED_DEFINITION,trim(EVS_SOURCE) EVS_SOURCE,
    trim(ORIGIN) ORIGIN, trim(SOURCE_DEFINITION) SOURCE_DEFINITION ,CHANGE_NOTE from 
    SBREXT.MDSR_CONCEPTS_EXT_TEMP  T,
       (select distinct PREFERRED_NAME 
    from SBREXT.MDSR_CONCEPTS_EXT_TEMP
    minus
    select distinct A.PREFERRED_NAME 
    from SBREXT.MDSR_CONCEPTS_EXT_TEMP A,    
    SBREXT.CONCEPTS_EXT C    where A.PREFERRED_NAME=C.PREFERRED_NAME) N
    where T.PREFERRED_NAME=N.PREFERRED_NAME)
    ORDER BY LONG_NAME;
...
errmsg :='';
         ...
         IF i.PREFERRED_NAME is null or i.LONG_NAME is null  or i.PREFERRED_DEFINITION is null
          ...
          IF v_source < 1 and i.EVS_SOURCE is not null THEN
           ...
           --dbms_output.put_line(i.EVS_SOURCE||'');
            ...
            insert into MDSR_CONCEPTS_EXT_ERROR_LOG VALUES (errmsg,sysdate,i.PREFERRED_NAME,i.LONG_NAME ,i.PREFERRED_DEFINITION,i.EVS_SOURCE,i.ORIGIN,i.SOURCE_DEFINITION,'','UPLOAD_VALIDATE_CONCEPTS');
            commit;
            ELSE
                  dbms_output.put_line(i.LONG_NAME||'i.LONG_NAME');
  ...
  EXCEPTION
    WHEN OTHERS THEN
        ...
        dbms_output.put_line('errmsg3 - '||errmsg);
                  insert into MDSR_CONCEPTS_EXT_ERROR_LOG VALUES (errmsg,sysdate,i.PREFERRED_NAME,i.LONG_NAME ,i.PREFERRED_DEFINITION,
                  i.EVS_SOURCE, i.ORIGIN, i.SOURCE_DEFINITION,'','UPLOAD_VALIDATE_CONCEPTS');
 end;
  end loop;
