CREATE OR REPLACE PACKAGE SBREXT.MDSREDCAP_GENER_XML IS
call_procs BOOLEAN := TRUE;
--  Repository User-Defined Error Messages
NO_ERR VARCHAR2(2) := 'B';
PROCEDURE RECAP_INSERT_CSV (P_run_N IN NUMBER);
PROCEDURE RECAP_UPDATE_CSV(P_run_N in NUMBER);
PROCEDURE RECAP_UPDATE_CSV2(P_run_N in NUMBER);
PROCEDURE RedCapSaction_populate(P_run_N in NUMBER);
PROCEDURE RedCapSact_Quest_populate(P_run_N in NUMBER);
PROCEDURE RedCapSaction_Insert(P_run_N in NUMBER);
PROCEDURE RedCapForm_Insert(P_run_N in NUMBER);
PROCEDURE RedCap_VALVAL_Insert(P_run_N in NUMBER);

END MDSREDCAP_GENER_XML;
/

CREATE OR REPLACE PACKAGE BODY SBREXT.MDSREDCAP_GENER_XML IS

PROCEDURE RECAP_INSERT_CSV(P_run_N number) as
...
from REDCAPPROTOCOL_TEMP;
  ...
  commit;
    ...
    errmsg := SQLERRM;
     ...
     commit;
     END RECAP_INSERT_CSV;

PROCEDURE RECAP_UPDATE_CSV(P_run_N number) as
...
commit;
 ...
 commit;
    ...
    errmsg := SQLERRM;
     ...
     commit;
     END RECAP_UPDATE_CSV;
PROCEDURE RECAP_UPDATE_CSV2(P_run_N number) as
...
commit;
    ...
    errmsg := SQLERRM;
       ...
       insert into MDSR_QUEST_CONTENTS_UPDATE_ERR values('MDSR_RECAP_UPDATE_CSV','', errmsg ,SYSDATE);
     commit;
     END RECAP_UPDATE_CSV2;

PROCEDURE  RedCapSaction_populate(P_run_N number) 
AS
...
END  RedCapSaction_populate;

PROCEDURE RedCapSact_Quest_populate(P_run_N number)
AS
 ...
 commit;
  EXCEPTION
                    ...
                    rollback;
                     ...
                     commit;
 END RedCapSact_Quest_populate;
            
 PROCEDURE  RedCapSaction_Insert(P_run_N number) 
AS
...
END RedCapSaction_Insert;           


PROCEDURE  RedCapForm_Insert(P_run_N number) 
AS
...
END RedCapForm_Insert;

PROCEDURE RedCap_VALVAL_Insert(P_run_N number)
AS
