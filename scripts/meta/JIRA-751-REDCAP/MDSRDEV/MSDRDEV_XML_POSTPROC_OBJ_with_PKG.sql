...
create index MSDRDEV.MDSR_PROTO_Q_INDX on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV (PROTOCOL);
create index MSDRDEV.MDSR_FN_Q_INDX on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV (form_name_new);
create index MSDRDEV.MDSR_FQ_INDX on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV (question);
create index MSDRDEV.MDSR_FSEC_INDX on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV (section_seq);
create index MSDRDEV.MDSR_FSEC_Q_INDX on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV (section_q_seq);
create index MSDRDEV.MDSR_QLOAD_Q_INDX on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV (LOAD_SEQ);
CREATE TABLE  MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR
(
  QC_ID         NUMBER,
...
CREATE TABLE  MSDRDEV.MDSR_QUEST_CONTENTS_REDCAP_BK
(
...
CREATE TABLE  MSDRDEV.MDSR_AFTERUPLOD_MODIFIED_REC
(
  QC_ID         NUMBER,
  VERSION               NUMBER(4,2),
...
GRANT SELECT on MSDRDEV.MDSR_AFTERXMLUPLOD_MODIFIED_REC TO PUBLIC;
GRANT SELECT on MSDRDEV.MDSR_REDCAP_PROTOCOL_CSV TO PUBLIC;
GRANT SELECT on MSDRDEV.MDSR_QUEST_CONTENTS_REDCAP_BK TO PUBLIC;
GRANT SELECT on MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR TO PUBLIC;

GRANT SELECT on MSDRDEV.MSDREDCAP_FORM_CSV TO PUBLIC;
GRANT SELECT on MSDRDEV.MSDREDCAP_SECTION_CSV TO PUBLIC;
GRANT SELECT on MSDRDEV.MSDREDCAP_VALUE_CODE_CSV TO PUBLIC;

create index MSDRDEV.MDSR_VV_PROTO_Q_INDX on MSDRDEV.MSDREDCAP_VALUE_CODE_CSV (PROTOCOL);
create index MSDRDEV.MDSR_VVFN_Q_INDX on MSDRDEV.MSDREDCAP_VALUE_CODE_CSV (form_name);
create index MSDRDEV.MDSR_VVQ_INDX on MSDRDEV.MSDREDCAP_VALUE_CODE_CSV (question);
create index MSDRDEV.MDSR_VVQLOAD_Q_INDX on MSDRDEV.MSDREDCAP_VALUE_CODE_CSV (LOAD_SEQ);

CREATE OR REPLACE FORCE VIEW  MSDRDEV.MDSR_DUP_QUESTION_FROM_XML_VW
(
    QC_ID,
    VERSION,
    PREFERRED_NAME,
...
select qc_id,
    ...
    ORDER BY 1,2,3;
/
GRANT SELECT on MSDRDEV.MDSR_DUP_QUESTION_FROM_XML_VW TO PUBLIC;
/
GRANT SELECT on MSDRDEV.MDSR_DUP_QUESTION_FROM_XML_VW TO PUBLIC
/

CREATE OR REPLACE PACKAGE MSDRDEV.MDSREDCAP_XML_POSTPOC IS
...
/
CREATE OR REPLACE PACKAGE BODY MSDRDEV.MDSREDCAP_XML_POSTPOC IS
...
BEGIN
      ...
      insert into  MSDRDEV.MDSR_AFTERUPLOD_MODIFIED_REC
...
call_procs := TRUE;
  commit;
    ...
    errmsg := substr(SQLERRM,1,500);
         ...
         call_procs  := FALSE;
         rollback;
       insert into  MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR values('NA',' FNAME', errmsg ,SYSDATE);
     commit;
...
BEGIN
      ...
      commit;
      insert into  MSDRDEV.MDSR_QUEST_CONTENTS_REDCAP_BK
  ...
  commit;
    ...
    errmsg := substr(SQLERRM,1,500);
         dbms_output.put_line('errmsg insert - '||errmsg);
         rollback;     
       insert into  MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR values(rec.QC_ID,rec.QTL_NAME||' FNAME', errmsg ,SYSDATE);
     commit;
     END;
     END LOOP;
...
BEGIN
  ...
  commit;
    ...
    errmsg := substr(SQLERRM,1,500);
         dbms_output.put_line('errmsg insert - '||errmsg);
         rollback;     
       insert into  MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR values(rec.QC_ID,rec.QTL_NAME, errmsg ,SYSDATE);
     commit;
     END;
     END LOOP;
     END MODE_SECTION;
...
BEGIN
  ...
  commit;
    ...
    errmsg := substr(SQLERRM,1,500);
         dbms_output.put_line('errmsg insert - '||errmsg);
         rollback;     
       insert into  MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR values(rec.QC_ID,rec.QTL_NAME||' DEF', errmsg ,SYSDATE);
     commit;
     END;
     END LOOP;
     END FORM_PREFDEF;
...
BEGIN
 ...
 dbms_output.put_line('errmsg insert - '||NO_ERR);
    ...
    errmsg := substr(SQLERRM,1,500);
         ...
         call_procs  := FALSE;
         rollback;
       insert into  MSDRDEV.MDSR_QUEST_CONTENTS_UPDATE_ERR values('NA','POSTPROC_RUN', errmsg ,SYSDATE);
     commit;
