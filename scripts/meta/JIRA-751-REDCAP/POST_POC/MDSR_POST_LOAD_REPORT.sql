...
select distinct long_name,rf.FORM_NAME_NEW,rf.preferred_definition correct_def,q.preferred_definition,q.modified_by,q.DATE_MODIFIED,qc_id,qc_idseq,created_by,date_created,QTL_NAME,version
from sbrext.quest_contents_ext q,
sbrext.MSDREDCAP_FORM_CSV rf
...
select distinct qtl_name from sbrext.quest_contents_ext f;
--5.Find Form truncated instructions long name

select distinct f.long_name form_name,trim(instructions) correct_instructions,m.preferred_definition truncated_instructions,m.QC_IDSEQ,m.qc_id,m.VERSION,
m.modified_by,m.DATE_MODIFIED,m.created_by,m.date_created,m.QTL_NAME
from sbrext.quest_contents_ext f,
sbrext.quest_contents_ext m,
sbrext.MSDREDCAP_FORM_CSV r
...
--and protocol not in ( list from check 1)
and load_seq=1
and trim(instructions)<>trim(m.preferred_definition)
and NVL(m.modified_by,'FORMLOADER') ='FORMLOADER';

--6.Find from Sections with truncated section long name

select distinct form_name,trim(section_new) correct_mod_name,m.long_name incorrect_mod_name,m.preferred_definition,
m.display_order ,m.QC_IDSEQ,m.qc_id,m.VERSION,m.QTL_NAME
from sbrext.quest_contents_ext f,
sbrext.quest_contents_ext m,
...
--7.Find from Questions with truncated Question long name 
select distinct form_name_new,q.long_name long_name,form_question correct_question,instructions,
...
--8.Find from Questions with not matching long name 
...
--9.Find Question Instructions with truncated long name
select form_name_new,m.long_name mod_name,form_question correct_question,instructions||'; Question in xml doesn''t contain valid CDE public id and version. Unable to validate question in xml.' correct_instructions,
i.long_name incorrect_instr,m.display_order mod_order, i.PREFERRED_DEFINITION,
i.qc_id,i.VERSION,i.QC_IDSEQ,i.QTL_NAME,i.MODIFIED_BY
from sbrext.quest_contents_ext f,
sbrext.MDSR_REDCAP_PROTOCOL_CSV r,
sbrext.quest_contents_ext  q,
...
--11. if not all questions where loaded ;
