...
FROM
(select   FIN_VM ,VM_IDSEQ FIN_IDSEQ,VM_ID,VM_IDSEQ, CONCEPTS_CODE,CONCEPTS_NAME,VM_NAME,CONDR_IDSEQ
from
 (select  max(VM_ID) over  (partition by CONDR_IDSEQ order by CONDR_IDSEQ ) as FIN_VM
...
select distinct CONDR_IDSEQ,CONCEPTS_CODE
 ...
 AND MDSR_GET_CONCEPT_SYNONYM(NRF.NAME,vm.LONG_NAME)=1))

 WHERE FIN_VM=VM_ID
 MINUS
 select   FIN_VM ,VM_IDSEQ FIN_IDSEQ,VM_ID,VM_IDSEQ, CONCEPTS_CODE,CONCEPTS_NAME,long_NAME,CONDR_IDSEQ
 FROM SBREXT.MDSR_VM_DUP_REF where  instr(CONCEPTS_CODE,':')=0)
...
(FIN_VM,FIN_IDSEQ ,VM_ID,VM_IDSEQ,CONCEPTS_CODE,CONCEPTS_NAME,LONG_NAME,CONDR_IDSEQ,CONCEPT_SYNONYM ,DATE_INSERTED)
 ...
 and (ASL_NAME) not like '%RETIRED%'
 AND FIN_VM<>VM.VM_ID
 AND MDSR_GET_CONCEPT_SYNONYM(RF.CONCEPTS_CODE,vm.LONG_NAME)=1
...
(
select NRF.CONDR_IDSEQ,NRF.NAME CONCEPTS_CODE,VM_ID,VM_IDSEQ,VM.LONG_NAME VM_NAME ,CONCEPTS_NAME
from
SBR.VALUE_MEANINGS VM,
 ---
...
(select a.CONDR_IDSEQ,a.NAME from
 ...
 select distinct CONDR_IDSEQ,CONCEPTS_CODE
 FROM SBREXT.MDSR_VM_DUP_REF where instr(CONCEPTS_CODE,'C45255')=0
 AND TRIM(UPPER(CONCEPTS_NAME))=TRIM(UPPER(LONG_NAME))
...
(FIN_VM,FIN_IDSEQ ,VM_ID,VM_IDSEQ,CONCEPTS_CODE,CONCEPTS_NAME,LONG_NAME,CONDR_IDSEQ,DATE_INSERTED ,CONCEPT_SYNONYM)
 select   FIN_VM ,VM_IDSEQ FIN_IDSEQ,VM_ID,VM_IDSEQ, CONCEPTS_CODE,CONCEPTS_NAME,long_NAME,CONDR_IDSEQ,SYSDATE,'NON'
 FROM
...
EXCEPTION
    ...
    WHEN others THEN
     V_error := substr(SQLERRM,1,200);
      insert into SBREXT.MDSR_VM_DUP_ERR VALUES('MDSR_VM_DUP_ERR', 'SBR.MDSR_VM_DUP_REF','STEP 4B','NA','NA',V_error,sysdate );
  commit;
end;
...
EXCEPTION
    WHEN others THEN
     V_error := substr(SQLERRM,1,200);
      insert into SBREXT.MDSR_VM_DUP_ERR VALUES('MDSR_VM_DUP_ERR', 'SBREXT.MDSR_VM_DUP_REF','STEP 5','NA','NA',V_error,sysdate );
  commit;
end;
...
EXCEPTION
    ...
    WHEN others THEN
     V_error := substr(SQLERRM,1,200);
      insert into SBREXT.MDSR_VM_DUP_ERR VALUES('MDSR_VM_DUP_ERR', 'SBR.MDSR_VM_DUP_REF','STEP 6','NA','NA',V_error,sysdate );
  commit;
end;
