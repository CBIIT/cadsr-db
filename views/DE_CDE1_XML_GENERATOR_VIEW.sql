...
CREATE OR REPLACE TYPE SBREXT."CDEBROWSER_VD_T2"                                          AS OBJECT
( "PublicId"         NUMBER,
  ...
  "PermissibleValues"    DE_VALID_VALUE_LIST_T,
  "ValueDomainConcepts"    Concepts_list_t
);
/


GRANT EXECUTE, DEBUG ON SBREXT.CDEBROWSER_VD_T2 TO CDEBROWSER;

GRANT EXECUTE, DEBUG ON SBREXT.CDEBROWSER_VD_T2 TO DER_USER;


/* Formatted on 8/22/2014 10:48:45 AM (QP5 v5.115.810.9015) */
CREATE OR REPLACE FORCE VIEW SBREXT.DE_CDE1_XML_GENERATOR_VIEW
(
   DE_IDSEQ,
   PUBLICID,
   LONGNAME,
               ...
               CAST (
                  MULTISET(SELECT   pv.VALUE,
                            ...
                            WHERE       vp.vd_idseq = vd.vd_idseq
                                    ...
                                    AND pv.vm_idseq = vm.vm_idseq                                                                      ) AS DE_VALID_VALUE_LIST_T
               ),
               CAST (
                  MULTISET(  SELECT   con.preferred_name,
                                      con.long_name,
...
GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON SBREXT.DE_CDE1_XML_GENERATOR_VIEW TO CDEBROWSER;

GRANT DELETE, INSERT, SELECT, UPDATE ON SBREXT.DE_CDE1_XML_GENERATOR_VIEW TO DER_USER;

GRANT SELECT ON SBREXT.DE_CDE1_XML_GENERATOR_VIEW TO DEV_READ_ONLY;

GRANT DELETE, INSERT, REFERENCES, SELECT, UPDATE, ON COMMIT REFRESH, QUERY REWRITE, DEBUG, FLASHBACK ON SBREXT.DE_CDE1_XML_GENERATOR_VIEW TO SBR WITH GRANT OPTION;