...
cursor c_load is
 ...
 (select count(*) cnt,trim(PREFERRED_NAME) PREFERRED_NAME,  trim(LONG_NAME) LONG_NAME,
  trim(PREFERRED_DEFINITION)PREFERRED_DEFINITION,trim(NVL(EVS_SOURCE,'NCI_CONCEPT_CODE'))EVS_SOURCE,
...
SELECT count(*) into v_source
            ...
            WHERE   trim(CONCEPT_SOURCE) = trim(i.EVS_SOURCE);
             ...
             IF v_source < 1 THEN
            errmsg :='Invalid SOURCE '||i.EVS_SOURCE||',';
            else
            errmsg :='';
            end if;
             ...
             insert into SBREXT.CONCEPTS_EXT
              ...
              VALUES
                  ...
                  (v_CON_IDSEQ,                    --CON_IDSEQ            NOT NULL,
                   ...
                   CASE when i.EVS_SOURCE ='NCI_CONCEPT_CODE' then  'NCI' else i.SOURCE_DEFINITION   end,  --DEFINITION_SOURCE
                   sysdate,                       --DATE_CREATED
                   CASE when i.EVS_SOURCE = 'NCI_CONCEPT_CODE' then  'NCI Thesaurus' else i.ORIGIN END,         --ORIGIN
                   'SBREXT',                      -- CREATED_BY
